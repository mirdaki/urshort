...
async fn main() {
    ...
    let short_url = warp::path::param()
        .and(warp::get())
        .and(with_url_mappings(urls))
        .and_then(get_match)
        .or(warp::get())
        .and_then(get_not_found);
...
async fn get_not_found() -> Result<impl warp::Reply, Infallible> {
    Ok("URShort is running!")
}

fn with_url_mappings(
    urls: UrlMappings,
) -> impl Filter<Extract = (UrlMappings,), Error = std::convert::Infallible> + Clone {
